...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Hiển thị toast cho mượt
      showToast(`Chào mừng ${data.full_name} đã quay trở lại!`, 'success');
            ...
            <header className="flex items-center justify-between whitespace-nowrap border-b border-solid border-slate-200 dark:border-slate-800 px-6 lg:px-40 py-4 bg-white dark:bg-slate-900">
              ...
              <div className="hidden md:flex flex-1 justify-end gap-8">
                <nav className="flex items-center gap-8">
                  <button type="button" className="text-slate-600 dark:text-slate-400 hover:text-primary dark:hover:text-primary text-sm font-medium transition-colors cursor-pointer">Sản phẩm</button>
                  <button type="button" className="text-slate-600 dark:text-slate-400 hover:text-primary dark:hover:text-primary text-sm font-medium transition-colors cursor-pointer">Khuyến mãi</button>
                  <button type="button" className="text-slate-600 dark:text-slate-400 hover:text-primary dark:hover:text-primary text-sm font-medium transition-colors cursor-pointer">Tin tức</button>
                  <button type="button" className="text-slate-600 dark:text-slate-400 hover:text-primary dark:hover:text-primary text-sm font-medium transition-colors cursor-pointer">Hỗ trợ</button>
                </nav>
              </div>
            </header>
              ...
              <div className="w-full max-w-[480px] bg-white dark:bg-slate-900 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
                ...
                <div className="p-8 lg:p-10">
                  ...
                  <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <button className="w-full bg-primary hover:bg-purple-800 disabled:bg-slate-400 disabled:cursor-not-allowed text-white font-bold py-4 rounded-lg shadow-lg shadow-primary/30 transition-all active:scale-[0.98] flex items-center justify-center gap-2" type="submit" disabled={loading}>
                      {loading ? (
                        <>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <button type="button" className="flex items-center justify-center gap-3 py-3 px-4 border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors font-medium text-slate-700 dark:text-slate-300">
            ...
            <footer className="px-6 lg:px-40 py-8 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-200 dark:border-slate-800">
              <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <div className="flex gap-6">
                  <button type="button" className="text-slate-400 hover:text-primary transition-colors text-sm cursor-pointer">Điều khoản</button>
                  <button type="button" className="text-slate-400 hover:text-primary transition-colors text-sm cursor-pointer">Bảo mật</button>
                  <button type="button" className="text-slate-400 hover:text-primary transition-colors text-sm cursor-pointer">Liên hệ</button>
                </div>
              </div>
            </footer>
          </div>
        ...
        {loginSuccess && (
          ...
          >
            <motion.div
              ...
              className="bg-white dark:bg-slate-900 rounded-3xl shadow-[0_32px_64px_-12px_rgba(0,0,0,0.3)] border border-slate-200 dark:border-slate-800 p-10 max-w-sm w-full text-center relative overflow-hidden"
            >
              {/* Background Glow */}
              <div className="absolute -top-24 -right-24 size-48 bg-emerald-500/10 rounded-full blur-3xl"></div>
              <div className="absolute -bottom-24 -left-24 size-48 bg-primary/10 rounded-full blur-3xl"></div>

              <div className="relative z-10">
                <div className="mb-6 inline-flex items-center justify-center size-24 bg-emerald-50 dark:bg-emerald-950/30 rounded-full text-emerald-500 shadow-inner">
                  <CheckCircle2 className="size-12 animate-in zoom-in duration-500 fill-emerald-500/10" />
                </div>
                
                <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">Đăng nhập thành công!</h2>
                <p className="text-slate-500 dark:text-slate-400 mb-8 leading-relaxed">
                  Hệ thống đang chuyển hướng bạn về trang chủ trong giây lát...
                </p>
                ...
                <div className="flex flex-col items-center gap-3">
                  ...
                  <span className="text-xs font-bold text-emerald-500 uppercase tracking-widest animate-pulse">Loading...</span>
                </div>
              </div>
            </motion.div>
          </motion.div>
